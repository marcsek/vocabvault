...
const TitleLayout = ({ children, buttonForSubmit, buttonPlaceholder, headingLeft }: Props) => {
  ...
  return (
    <section className="flex flex-col gap-6 md:gap-11">
      ...
      {SubmitButton && (
        <div className="fixed bottom-0 left-0 z-20 flex h-16 w-full items-center justify-center bg-gray-800/70 px-9 py-4 shadow-[0px_-2px_6px_1px_#00000033] backdrop-blur-sm md:hidden [&>*]:w-full">
          {SubmitButton}
        </div>
      )}
    </section>
