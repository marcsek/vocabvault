...
export const SelectedWordsDetail = ({ id, languageCode, pairsInGroup, selectedGroup }: NonNullable<TSelectedWords>) => {
  ...
  const { data: wordPairs, isLoading } = useGetWordSourceWordPairs(id, { takeSkip: { skip, take } });

  return (
    <div className="flex flex-grow flex-wrap gap-2 self-start p-4">
      <AnimatePresence>
        {isLoading ? (
          <motion.div className="self-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            Loading
          </motion.div>
        ) : (
          <>
